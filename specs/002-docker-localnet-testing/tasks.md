...
- [x] T001 Create `docker/` directory structure per plan.md (`docker/`, `docker/logs/.gitkeep`)
- [x] T002 [P] Generate deterministic test wallet keypair and save to `docker/test-wallet.json`
- [x] T003 [P] Create `docker/.env` with default environment variables (RPC_PORT, WS_PORT, POSTGRES_PORT, POSTGRES_DB, POSTGRES_USER, POSTGRES_PASSWORD) per data-model.md
- [x] T004 [P] Create `docker/.gitignore` to exclude `logs/*.log` but keep `logs/.gitkeep`
- [x] T005 [P] Add `docker/.dockerignore` to exclude `logs/`, `node_modules/`, `.env.local`
...
- [x] T006 Create `docker/Dockerfile` with `FROM --platform=linux/amd64 ubuntu:22.04`, install Solana v1.18.26 via direct tarball extract to `/opt/solana-release/`, install `tini`, `curl`, Node.js 20.x LTS, and npm dependencies (`@coral-xyz/anchor`, `@solana/web3.js`, `@solana/spl-token`, `tsx`) per research.md Q1, Q2, Q6
- [x] T007 Create `docker/package.json` with bootstrap script dependencies (`@coral-xyz/anchor`, `@solana/web3.js`, `@solana/spl-token`, `bn.js`, `tsx`) — installed during Docker build
...
- [x] T012 [US2] Create `docker/bootstrap.ts` — TypeScript script adapted from `scripts/devnet-validate.ts` that: (1) connects to `http://127.0.0.1:8899`, (2) loads IDL from `$IDL_PATH`, (3) derives GlobalState/Mint/MintAuthority PDAs, (4) checks if already initialized (idempotent), (5) calls `program.methods.initialize(params)` with default params from data-model.md, (6) airdrops 100 SOL to test wallet from `$WALLET_PATH`, (7) creates Token-2022 ATA for test wallet, (8) mints 10,000 HELIX via `admin_mint`
- [x] T013 [US2] Update `docker/entrypoint.sh` to invoke `npx tsx /app/bootstrap.ts` after `wait_for_ready()` succeeds, exit non-zero on bootstrap failure, touch `/tmp/.bootstrap-complete` sentinel only after successful bootstrap, print summary (RPC URL, WS URL, test wallet public key, balances)
- [x] T014 [US2] Update `docker/Dockerfile` to COPY `bootstrap.ts` and `package.json` into image, run `npm install --production` during build to pre-install Anchor/web3 dependencies
- [x] T015 [US2] Add pre-flight validation to `docker/entrypoint.sh` — check that `$PROGRAM_SO_PATH` exists and is non-empty before starting validator, print helpful error message if missing ("Run 'anchor build' first")
...
- [x] T016 [P] [US3] Add npm convenience scripts to root `package.json` — `localnet:up`, `localnet:up:all`, `localnet:down`, `localnet:logs`, `localnet:status` per contracts/cli-interface.md
...
- [x] T018 [US4] Add `postgres` service to `docker/docker-compose.yml` — `postgres:16-alpine` image, port mapping (`${POSTGRES_PORT:-5432}:5432`), environment variables (`POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`), healthcheck (`pg_isready`) per contracts/docker-compose-contract.md
- [x] T019 [US4] Add `.env` loading to `docker/docker-compose.yml` via `env_file: .env` directive so Compose picks up variable overrides from `docker/.env`
- [x] T020 [US4] Update `docker/README.md` with full-stack section — how to start both services, indexer connection instructions (`DATABASE_URL`, `RPC_URL`), how to run `npm run db:migrate` against local PostgreSQL
...
- [x] T021 [P] Add `docker/` section to root `README.md` — brief overview of localnet Docker tooling with link to `docker/README.md`
- [x] T022 [P] Verify edge case: container behavior when `helix_staking.so` is missing — confirm entrypoint prints error and exits non-zero (covered by T015 pre-flight check)
- [x] T023 Run quickstart.md validation — follow quickstart.md steps exactly on a clean checkout, verify all commands succeed, fix any discrepancies
- [x] T024 [P] Validate SC-002: start container, run a lightweight RPC smoke test script (`docker/smoke-test.ts`) that (1) calls `getVersion` to confirm Solana 1.18.x, (2) queries the program account at `E9B7BsxdPS89M66CRGGbsCzQ9LkiGv6aNsra3cNBJha7` and verifies it is executable, (3) submits an `initialize`-idempotent instruction via `@solana/web3.js`, (4) opens a WebSocket subscription on `ws://localhost:8900` and confirms a log event is received. All checks must pass without modifying any existing test code.
- [x] T025 [P] Validate SC-001/SC-003: measure cold start time (`time docker compose -f docker/docker-compose.yml up validator`), warm start time (after `down` + `up`), and container memory usage (`docker stats --no-stream`). Verify cold < 60s, warm < 15s, memory < 2 GB
