...
describe("Initialize", () => {
  it("initializes protocol with correct GlobalState parameters", async () => {
    ...
    expect(globalState.annualInflationBp.toString()).to.equal(
      DEFAULT_ANNUAL_INFLATION_BP.toString()
    );
    expect(globalState.minStakeAmount.toString()).to.equal(
      DEFAULT_MIN_STAKE_AMOUNT.toString()
    );
    expect(globalState.shareRate.toString()).to.equal(
      DEFAULT_STARTING_SHARE_RATE.toString()
    );
    expect(globalState.startingShareRate.toString()).to.equal(
      DEFAULT_STARTING_SHARE_RATE.toString()
    );
    expect(globalState.slotsPerDay.toString()).to.equal(
      DEFAULT_SLOTS_PER_DAY.toString()
    );
    expect(globalState.claimPeriodDays).to.equal(DEFAULT_CLAIM_PERIOD_DAYS);

    // Verify init_slot is set (should be > 0)
    expect(globalState.initSlot.toNumber()).to.be.greaterThan(0);

    // Verify all counters are initialized to zero
    expect(globalState.totalStakesCreated.toString()).to.equal("0");
    expect(globalState.totalUnstakesCreated.toString()).to.equal("0");
    expect(globalState.totalClaimsCreated.toString()).to.equal("0");

    // Verify aggregate metrics are zero
    expect(globalState.totalTokensStaked.toString()).to.equal("0");
    expect(globalState.totalTokensUnstaked.toString()).to.equal("0");
    expect(globalState.totalShares.toString()).to.equal("0");
    expect(globalState.currentDay.toString()).to.equal("0");
  });
  ...
  it("creates Token-2022 mint with correct configuration", async () => {
    ...
    expect(mintAccountInfo.owner.toBase58()).to.equal(TOKEN_2022_PROGRAM_ID.toBase58());
    ...
    expect(data.length).to.be.greaterThanOrEqual(82);
    ...
    expect(hasMintAuthority).to.equal(1); // Should have mint authority

    const mintAuthorityBytes = data.subarray(4, 36);
    const mintAuthority = new PublicKey(mintAuthorityBytes);
    expect(mintAuthority.toBase58()).to.equal(mintAuthorityPDA.toBase58());
  });
  ...
  it("rejects double initialization", async () => {
    ...
    try {
      ...
      expect.fail("Expected second initialization to fail, but it succeeded");
    } catch (error) {
  ...
  it("clock mocking works with Bankrun", async () => {
    ...
    expect(newClock.slot).to.equal(initialSlot + slotsPerDay);
    ...
    expect(actualTimestampDelta).to.equal(expectedTimestampDelta);
  });
});
