...
export function createEd25519Instruction(
  ...
  // [num_signatures, padding, sig_offset, sig_len, pk_offset, pk_len, msg_offset, msg_len, signature, publickey, message]
  const numSignatures = 1;
  const padding = 0;

  // Offsets (little-endian u16)
  const sigOffset = 16; // After the header
  const sigLen = 64;
  ...
  instructionData.writeUInt16LE(sigOffset, 2);
  instructionData.writeUInt16LE(sigLen, 4);
  instructionData.writeUInt16LE(pkOffset, 6);
  instructionData.writeUInt16LE(pkLen, 8);
  instructionData.writeUInt16LE(msgOffset, 10);
  instructionData.writeUInt16LE(msgLen, 12);
  // 2 bytes padding at 14-15
