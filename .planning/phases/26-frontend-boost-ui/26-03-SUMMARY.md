...
plan: "03"
subsystem: indexer, web
tags: [push-notifications, boost, events, drizzle, react]
dependency_graph:
  requires: [Phase 24 BoostRevoked event emission, Phase 12 push notification infrastructure]
  provides: [BoostRevoked push notification pipeline, notifyBoostRevoked user preference]
  affects: [services/indexer, app/web/components/dashboard]
tech_stack:
  added: []
  patterns: [event-driven notification dispatch, idempotent event deduplication]
key_files:
  created:
  ...
  modified:
    ...
    - services/indexer/src/__tests__/notification-scheduler.test.ts
decisions:
  - BoostRevoked uses event-driven dispatch (not scheduler): no periodic check needed since on-chain event is the trigger
  - boost_revoked_events table provides idempotent deduplication (onConflictDoNothing on signature)
  - notifyBoostRevoked defaults to true (opt-out model): boost revocation is high-urgency financial event
metrics:
  duration: 4066s
