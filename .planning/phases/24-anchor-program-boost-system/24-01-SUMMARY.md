...
tags: [anchor, rust, solana, staking, boost, pda]

# Dependency graph
requires:
  - phase: 23-communication-boost-rules
    provides: Boost rules UX/disclosure UI; boost system must be live before staking UI actions
provides:
...
affects:
  - 24-02 (register_seed_boost, create_stake with boost)
  - 24-03 (claim_rewards with boost, revocation crank)

# Tech tracking
tech-stack:
  added: []
  patterns:
    - GlobalState reserved-slot helpers pattern (get/set via LE u64 encoding for Pubkey)
    - PDA validation helper pattern (validate_*_pda in security/pda.rs)
    - Admin instruction pattern with authority constraint and re-callable semantics
    - TDD pattern for math functions (RED: write tests, GREEN: implement, verify all pass)
...
requirements-completed: [BOOST-01, BOOST-06]
