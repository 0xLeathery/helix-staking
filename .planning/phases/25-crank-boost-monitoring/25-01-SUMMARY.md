...
plan: "01"
subsystem: infra
tags: [solana, anchor, crank, spl-token, node-cron, boost, litesvm]

# Dependency graph
requires:
  - phase: 24-anchor-program-boost-system
    provides: update_boost_status permissionless instruction, BoostRecord PDA, GlobalState reserved[7] boost_enabled flag and reserved[2..5] seed_mint encoding
provides:
...
affects: [phase-26-frontend-boost-ui, phase-27-mainnet-launch]

# Tech tracking
tech-stack:
  added:
    - "@solana/spl-token@^0.4.14 (services/crank — for getAssociatedTokenAddressSync)"
  patterns:
    - "Sweep pattern: enumerate all PDAs, filter to active/linked, send permissionless tx per record"
    - "Fresh blockhash per transaction in sweep loop (CRANK-02 pattern)"
    - "Individual transaction failure caught and logged without aborting sweep"
    - "client.expireBlockhash() between duplicate LiteSVM calls to prevent AlreadyProcessed rejection"
...
requirements-completed: [BOOST-08]
